...
function readRhoManifestFile(packageJsonPath: string): RhoManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { rho?: RhoManifest };
		return pkg.rho ?? null;
	} catch {
		return null;
	}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readRhoManifestFile(packageJsonPath);
...
export class DefaultPackageManager implements PackageManager {
	...
	private collectPackageResources(
		...
		const manifest = this.readRhoManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof RhoManifest];
	...
	private readRhoManifest(packageRoot: string): RhoManifest | null {
